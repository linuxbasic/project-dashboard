...
class PhaseEndDateTests(TestCase):
    ...
    def test_should_return_planned_start_date_if_no_duration_predictions_available(self):
        ...
        self.assertEqual(phase1.get_end_date(), START_DATE + timedelta(days=PLANNED_DURATION))
    ...
    def test_should_return_predicted_start_date_on_specific_date(self):
        ...
        self.assertEqual(phase1.get_end_date(date.today() + timedelta(days=1)),
                         START_DATE + timedelta(days=PREDICTED_DURATION_1))
        self.assertEqual(phase1.get_end_date(date.today() + timedelta(days=2)),
                         START_DATE + timedelta(days=PREDICTED_DURATION_1))
        self.assertEqual(phase1.get_end_date(date.today() + timedelta(days=3)),
                         START_DATE + timedelta(days=PREDICTED_DURATION_2))
        self.assertEqual(phase1.get_end_date(date.today() + timedelta(days=4)),
                         START_DATE + timedelta(days=PREDICTED_DURATION_2))
