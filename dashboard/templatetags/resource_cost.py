...
def planned_resource_cost(resource, project):
    return sum([phase.get_planned_cost() for phase in resource.tasks.filter(phase__project=project) if
                phase.get_end_date() <= project.today()])


@register.filter
def resource_cost(resource, project):
    return sum([phase.get_cost() for phase in resource.tasks.filter(phase__project=project) if
                phase.get_end_date() <= project.today()])


@register.filter
def cost_delta(resource, project):
